...
class Memory:
    ...
    def get_context(self):
        if not self.data:
            return "Noch keine Infos über den Nutzer."
...
class Todos:
    ...
    def complete(self, index):
        data = self._load()
        open_todos = [t for t in data["todos"] if not t["done"]]
        if 0 <= index < len(open_todos):
            open_todos[index]["done"] = True
            self._save(data)
            return f"Erledigt: {open_todos[index]['text']}"
        return "Diese Aufgabe gibt es nicht."

    def clear(self):
        self._save({"todos": []})
        return "To-Do Liste gelöscht."